...
FAST_SERVER_URL = f"http://{config.FAST_VECTOR_STORE_HOST}:{config.FAST_VECTOR_STORE_PORT}"
SLOW1_SERVER_URL = f"http://{config.SLOW1_VECTOR_STORE_HOST}:{config.SLOW1_VECTOR_STORE_PORT}"
SLOW2_SERVER_URL = f"http://{config.SLOW2_VECTOR_STORE_HOST}:{config.SLOW2_VECTOR_STORE_PORT}"
...
def get_all_server_status():
    return check_server_status(FAST_SERVER_URL), check_server_status(SLOW1_SERVER_URL), check_server_status(SLOW2_SERVER_URL)
...
def manage(past_base_data_directory):
    ...
    fast_server_status, slow1_server_status, slow2_server_status = get_all_server_status()
    ...
    fast_data_directory_pending = list(set(curr_base_data_directory) - set(curr_fast_data_directory))
    slow1_data_directory_pending = list(set(curr_base_data_directory) - set(curr_slow1_data_directory))
    slow2_data_directory_pending = list(set(curr_base_data_directory) - set(curr_slow2_data_directory))
    ...
    if fast_server_status:
        ...
        if len(fast_data_directory_pending) > 0:
            ...
            os.link(os.path.join(BASE_DATA_DIRECTORY, file), os.path.join(FAST_SERVER_DATA_DIRECTORY, file))
    ...
    if slow1_server_status and slow2_server_status:
        ...
        if added_to_slow1:
            ...
            os.system(f"cp -ruT {SLOW1_SERVER_CACHE_DIRECTORY} {SLOW2_SERVER_CACHE_DIRECTORY}")
            os.link(os.path.join(BASE_DATA_DIRECTORY, file_added_to_slow1), os.path.join(SLOW2_SERVER_DATA_DIRECTORY, file_added_to_slow1))
        ...
        # never used
            ...
            os.system(f"cp -ruT {SLOW2_SERVER_CACHE_DIRECTORY} {SLOW1_SERVER_CACHE_DIRECTORY}")
            os.link(os.path.join(BASE_DATA_DIRECTORY, file_added_to_slow2), os.path.join(SLOW1_SERVER_DATA_DIRECTORY, file_added_to_slow2))
        ...
        # SLow1 will go down if file is linked
            ...
            diff_of_slow1_slow2_data = list(set(curr_slow1_data_directory) - set(curr_slow2_data_directory))
            ...
            if len(slow1_data_directory_pending) > 0:
                ...
                os.link(os.path.join(BASE_DATA_DIRECTORY, file), os.path.join(SLOW1_SERVER_DATA_DIRECTORY, file))
                added_to_slow1 = True
