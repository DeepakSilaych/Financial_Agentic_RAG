...
def extract_company_and_year(prompt):
    ...
    normalized_prompt = " ".join(word.capitalize() if word.isalpha() else word for word in prompt.split())
    
    # Process the prompt with spaCy
    doc = nlp(normalized_prompt)
    
    # Extract year (first 4-digit number in the text)
    year = next((word.text for word in doc if word.is_digit and len(word.text) == 4), None)
