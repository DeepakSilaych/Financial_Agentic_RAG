...
def rewrite_question(state: state.InternalRAGState):
    ...
    if len(state["documents"]) + len(state.get("documents_with_kv", [])) == 0 and (
        prev_node_rewrite.split('//')[0] == nodes.retrieve_documents_with_metadata.__name__
        or prev_node_rewrite.split('//')[0] == nodes.retrieve_documents_with_quant_qual.__name__
    ):
        metadata_retries += 1
    ...
    if not LOGGING_SETTINGS["rewrite_question"] or state.get("send_log_tree_logs" , "") == "False":
...
def rewrite_with_hyde(state: state.InternalRAGState):
    ...
    if not LOGGING_SETTINGS["rewrite_with_hyde"] or state.get("send_log_tree_logs" , "") == "False":
        child_node = parent_node
