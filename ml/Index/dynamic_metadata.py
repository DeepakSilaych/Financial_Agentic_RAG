...
class TableValuesSchema(BaseModel):
    succint_context: str = Field(description="A short succinct context to situate this chunk within the overall document.")
    listofstr: List[str] | str = Field(description="List of strings where each string is having description of a financial number/value from the TABLE in brief including all the context(Table name, row name, column name) it has for the value with the exact value.")
    ...
    item_10K: ITEMS_10K_LITERALS | str | None = Field(description="The item no. with name if it is a 10-K document otherwise None.")
    
class ListOfStrSchema(BaseModel):
    listofstr: List[str] = Field(description="Given a string representation of a list, return a list of strings by correctly parsing the string representation of the list.")
...
class OtherDynamicMetadataSchema(BaseModel):
    succint_context: str = Field(description="A short succinct context to situate this chunk within the overall document.")
    topic: str = Field(description="One broad Topic from the list of broad topics you have assigned already to some chunks of this document, that best describes the content of this chunk. If you cannot find any topic that fits this chunk, then return a new BROAD topic that best describes the content of this chunk.")
    ...
    if type == "10-K":
        if typetext == "table":
            ...
            if not isinstance(keyvals, list):
                ...
                keyvals = llm.with_structured_output(ListOfStrSchema).invoke(prompt).listofstr
...
def situate_context_finance(doc: str, chunk: str, typetext: str, type: str):
        ...
        messages=[
            {
                ...
                "content": [
                    ...
                    {
                        "type": "text",
                        "text": GLOBAL_SET_OF_FINANCE_TERMS_PROMPT.format(finance_terms = "\n".join(GLOBAL_SET_OF_FINANCE_TERMS)),
                        "cache_control": {"type": "ephemeral"}
                    },
                    {
                        "type": "text",
                        "text": FINANCE_CHUNK_CONTEXT_PROMPT.format(chunk_content=chunk),
                    }
                ]
            }
        ],
    ...
    validated_response = validate_and_process_finance_response(response[0], typetext, type)
    return [validated_response]

def situate_context_finance_table(doc: str, chunk: str, prev_chunk: str, typetext: str, type: str):
    response = client.chat.completions.create_with_completion(
        model="claude-3-haiku-20240307",
        max_tokens=4096,
        temperature=0.0,
        messages=[
            {
                ...
                "content": [
                    ...
                    {
                        "type": "text",
                        "text": FINANCE_TABLE_CHUNK_CONTEXT_PROMPT.format(chunk_content=chunk, chunk_before_table=prev_chunk),
                    }
                ]
            }
        ],
        ...
        messages=[
            {
                ...
                "content": [
                    ...
                    {
                        "type": "text",
                        "text": SET_OF_TERMS_USED_TILL_NOW.format(topics = "\n".join(set_of_topics)),
                        "cache_control": {"type": "ephemeral"}
                    },
                    {
                        "type": "text",
                        "text": OTHER_CHUNK_CONTEXT_PROMPT.format(chunk_content=chunk, num_topics=len(set_of_topics)),
                    }
                ]
            }
        ],
