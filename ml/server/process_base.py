...
class BaseMessageProcessor:
    ...
    async def save_user_message(self, chat_id: int, message_text: str, websocket, db: Session):
    ...
    async def handle_intermediate_message(self, chat_id: int, question: dict, websocket, db: Session):
        ...
        try:
            while True:
                ...
                if response['type'] == 'clarification_response':
                    ...
                    db.commit()
                    db.refresh(intermediate_message)
                    logger.info(f"Received clarification response: {response['answer']}")
                    return response['answer']
        except WebSocketDisconnect:
            logger.warning("WebSocket disconnected while waiting for clarification response")
        
        return 

    async def handle_response(self, chat_id: int, response_content: str, websocket, db: Session, charts=None):
