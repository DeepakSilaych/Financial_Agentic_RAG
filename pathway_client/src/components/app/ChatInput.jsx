...
const ChatInput = ({ setError: setParentError, chatId, messages, onSendMessage }) => {
  ...
  const [selectedLLM, setSelectedLLM] = useState('claude-3-haiku');
  ...
  const handleSendMessage = async (e) => {
    ...
    if (message.trim()) {
      try {
        setIsLoading(true);
        await onSendMessage(message.trim(), mode, researchMode, selectedFiles, selectedLLM);
        setMessage('');
        setSelectedFiles([]);
        setIsExpanded(false);
      } catch (error) {
  ...
  const LLMSelect = () => {
    const LLM_OPTIONS = [
      ...
      { value: 'claude-3-sonnet', label: 'Claude Sonnet' },
      { value: 'claude-3-opus', label: 'Claude Opus' }
    ];
    ...
    return (
      <Select.Root value={selectedLLM} >
        ...
        >
          <Select.Value>
            {LLM_OPTIONS.find(option => option.value === selectedLLM)?.label || 'Select LLM'}
          </Select.Value>
          <Select.Icon>
            ...
            <div className="flex items-center gap-2">
              <div className="relative">
                <div className="flex items-center">
                  <span className="mr-2 text-sm text-gray-600">Fast mode:</span>
                  <label className="relative inline-flex items-center cursor-pointer">
                    <input
                      type="checkbox"
                      className="sr-only peer"
                      checked={mode === 'fast'}
                      onChange={() => {setMode(mode === 'fast' ? 'slow' : 'fast'); console.log(mode);}}
                    />
              ...
              >
                <Paperclip size={18} />
                <span className={selectedFiles.length > 0 ? "text-blue-600 font-medium" : ""}>
                  {selectedFiles.length > 0 ? `${selectedFiles.length} file(s)` : "Attach files"}
                </span>
              </button>
            </div>
